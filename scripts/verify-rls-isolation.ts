...
async function main() {
  ...
  try {
    ...
    const clientASetup = await batch([
      ...
      db.insert(clients).values({ agencyId: agencyAId, name: "Client A", phone: "+10000000001" }).returning({ id: clients.id }),
    ]);
    ...
    const clientBSetup = await batch([
      db.execute(sql`SELECT set_config('app.agency_id', ${agencyBId}, true)`),
      db.execute(sql`SELECT set_config('app.role', 'admin', true)`),
      db.insert(clients).values({ agencyId: agencyBId, name: "Client B", phone: "+10000000002" }).returning({ id: clients.id }),
    ]);
