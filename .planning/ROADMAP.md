...
Plans:
- [ ] 04-01-PLAN.md — Migraciones 0014/0015: cerrar los dos gaps reales de RLS (`audit_log` sin ramas de rol, `messages` sin chequeo de `client_assignments`) + suite `verify-agent-rls.ts` (SEG-11, SIS-01, SEG-06, SEG-08)
- [ ] 04-02-PLAN.md — Dependencias, contrato de env, cliente Anthropic vía passthrough de OpenRouter y system prompt con SEG-09 inyectado sin ramas (SEG-09, SEG-07, SEG-05)
- [ ] 04-03-PLAN.md — Tabla `approval_queue` + migración 0016 (RLS solo-equipo) y columnas `status`/`approval_id` en `audit_log` (SEG-10, SEG-11)
- [ ] 04-04-PLAN.md — `buildAgentContext`: el único lector de contexto por turno, con tope de historial y cero filtros de cliente en aplicación (SEG-05/06/07/08, SIS-01)
- [ ] 04-05-PLAN.md — Descarga de media en dos pasos desde Meta, transcripción Deepgram Nova-3 e `interpretMedia` compartido (WA-05)
- [ ] 04-06-PLAN.md — Registro de herramientas + interceptor de riesgo SEG-10 (bajo ejecuta, alto encola) con cross-check de alcance (SEG-10, SEG-11, SEG-07)
- [ ] 04-07-PLAN.md — `runTurn` acotado + `process-agent-turn` de Inngest reemplazando el ack fijo de la Fase 3 (WA-05, SEG-09, SEG-10, SIS-01, SEG-05)
- [ ] 04-08-PLAN.md — `scripts/verify-agent.ts`: suite de integración contra Neon real del motor SEG-10 y la frontera de contexto (SEG-10, SEG-11, SEG-05/06/07)
- [ ] 04-09-PLAN.md — Server Actions de aprobación/rechazo + función Inngest que reproduce la acción aprobada (SEG-10, SEG-11)
- [ ] 04-10-PLAN.md — Chat web solo-equipo: migración 0017, `POST /api/chat` y página de conversación multimodal (WA-05, SIS-01, SEG-09, SEG-05)
